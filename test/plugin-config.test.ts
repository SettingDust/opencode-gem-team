...
describe("Gem Team config hook injection", () => {
  ...
  it("appends the delegation-first notice to gem-orchestrator prompt", () => {
    ...
    assert.ok(prompt.includes("Emitting the block is NOT the action"))
  ...
  it("uses the complexity-driven decision block fields in the correct order", () => {
    ...
    assert.ok(GEM_ORCHESTRATOR_PROMPT_NOTICE.includes("Action MUST be exactly one of the values listed above - never \`delegate\`, \`task\`, or any other freeform verb."))
    assert.ok(GEM_ORCHESTRATOR_PROMPT_NOTICE.includes("Emitting the block is NOT the action."))
