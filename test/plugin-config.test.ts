...
describe("Gem Team config hook injection", () => {
  ...
  it("appends the delegation-first notice to gem-orchestrator prompt", () => {
    ...
    assert.ok(prompt.includes("Action is one or more of the listed verbs (combine with ` + `)."))
  ...
  it("uses the complexity-driven decision block fields in the correct order", () => {
    ...
    const decisionIndex = GEM_ORCHESTRATOR_PROMPT_NOTICE.indexOf("Decision: <delegate to which subagent or handle directly, per your Complexity + Action + agent rules>")
    const actionNoteIndex = GEM_ORCHESTRATOR_PROMPT_NOTICE.indexOf("Action is one or more of the listed verbs (combine with ` + `).")
