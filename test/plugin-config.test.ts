...
describe("Gem Team config hook injection", () => {
  ...
  it("appends the delegation-first notice to gem-orchestrator prompt", () => {
    ...
    assert.ok(prompt.includes("The decision block is a PREFACE, never the deliverable"))
    ...
    assert.ok(prompt.includes("Correct pattern:"))
    assert.ok(prompt.includes("[then IMMEDIATELY call task() in the same turn - do not stop here]"))
    assert.ok(prompt.includes("Never output a decision block as your final message."))
  ...
  it("uses the complexity-driven decision block fields in the correct order", () => {
    ...
    assert.ok(GEM_ORCHESTRATOR_PROMPT_NOTICE.includes("The decision block is a PREFACE, never the deliverable, and is worthless on its own."))
    assert.ok(GEM_ORCHESTRATOR_PROMPT_NOTICE.includes("the very next thing after the block MUST be the actual tool call"))
    assert.ok(GEM_ORCHESTRATOR_PROMPT_NOTICE.includes("Never output a decision block as your final message."))
