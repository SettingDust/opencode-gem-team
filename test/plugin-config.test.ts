...
describe("Gem Team config hook injection", () => {
  ...
  it("appends the delegation-first notice to gem-orchestrator prompt", () => {
    ...
    assert.ok(prompt.includes("output a decision block"))
    ...
    assert.ok(prompt.includes("Action is one or more of the listed verbs (combine with ` + `). Keep Decision to that choice only; explain any other context in normal prose after the block."))
    ...
    assert.ok(prompt.includes("Decision: <per your Complexity + Action + agent rules, start it yourself or delegate to which subagent>"))
  ...
  it("uses the complexity-driven decision block fields in the correct order", () => {
    ...
    const decisionIndex = GEM_ORCHESTRATOR_PROMPT_NOTICE.indexOf("Decision: <per your Complexity + Action + agent rules, start it yourself or delegate to which subagent>")
    const actionNoteIndex = GEM_ORCHESTRATOR_PROMPT_NOTICE.indexOf("Action is one or more of the listed verbs (combine with ` + `). Keep Decision to that choice only; explain any other context in normal prose after the block.")
    ...
    assert.notEqual(actionNoteIndex, -1)
    assert.notEqual(checkpointIndex, -1)
    assert.equal(GEM_ORCHESTRATOR_PROMPT_NOTICE.includes("the very next thing after the block MUST be the actual tool call"), false)
    assert.equal(GEM_ORCHESTRATOR_PROMPT_NOTICE.includes("Never output a decision block as your final message."), false)
    assert.equal(GEM_ORCHESTRATOR_PROMPT_NOTICE.includes("Correct pattern:"), false)
    ...
    assert.ok(decisionIndex < actionNoteIndex)
    assert.ok(actionNoteIndex < checkpointIndex)
  })
