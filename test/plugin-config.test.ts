...
describe("Gem Team config hook injection", () => {
  ...
  it("appends the delegation-first notice to gem-orchestrator prompt", () => {
    ...
    assert.ok(prompt.includes("perform that action in the SAME turn"))
    assert.ok(prompt.includes("The decision block is narration only and does not change or block what you do next - just continue performing the action as normal."))
    ...
    assert.ok(prompt.includes("Action MUST be one or more of the values listed above"))
    assert.ok(prompt.includes("combine multiple with ` + `, e.g. `debug + implement`"))
  ...
  it("uses the complexity-driven decision block fields in the correct order", () => {
    ...
    assert.ok(GEM_ORCHESTRATOR_PROMPT_NOTICE.includes("Action MUST be one or more of the values listed above (combine multiple with \` + \`, e.g. \`debug + implement\`) - never \`delegate\`, \`task\`, or any other freeform verb."))
    assert.ok(GEM_ORCHESTRATOR_PROMPT_NOTICE.includes("The decision block is narration only and does not change or block what you do next - just continue performing the action as normal."))
