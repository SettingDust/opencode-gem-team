...
- preserves any user-defined OpenCode `agent.model` fields without adding plugin-side model routing
...
- Existing OpenCode `agent.model` values are preserved during agent injection.
- The plugin does not expose model routing options, per-agent overrides, role model maps, or provider-side routing.
