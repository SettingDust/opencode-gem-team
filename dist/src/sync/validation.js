...
export function validateManifest(manifest) {
    ...
    if (!manifest.source?.ref)
        errors.push("manifest.source.ref is required");
    if (!COMMIT_SHA_PATTERN.test(manifest.source?.commit ?? ""))
        errors.push("manifest.source.commit must be a resolved commit SHA");
...
function validateEntry(entry, manifest, errors) {
    ...
    if (entry.upstreamRef !== manifest.source?.ref)
        errors.push(`${entry.slug}: upstreamRef must match manifest source ref`);
    if (entry.upstreamCommit !== manifest.source?.commit)
        errors.push(`${entry.slug}: upstreamCommit must match manifest source commit`);
    if (!COMMIT_SHA_PATTERN.test(entry.upstreamCommit ?? ""))
        errors.push(`${entry.slug}: upstreamCommit must be a resolved commit SHA`);
