...
export function validateGemTeamConfig(config: Record<string, unknown> | GemTeamPluginOptions): GemTeamConfigValidationResult {
  const errors: string[] = []

  for (const key of Object.keys(config)) {
    if (unsupportedConfigKeys.has(key)) errors.push(`unsupported_config_key:${key}`)
  }

  const complexityModels = (config as GemTeamPluginOptions).complexity_models
  if (complexityModels !== undefined) {
    for (const key of Object.keys(complexityModels as Record<string, unknown>)) {
...
function modelForTier(models: ComplexityModels | undefined, tier: keyof ComplexityModels): string | undefined {
