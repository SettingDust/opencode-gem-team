...
export function validateManifest(manifest: SyncedAgentsManifest): SyncValidationResult {
  ...
  if (!isIsoDateTime(manifest.syncedAt)) errors.push("manifest.syncedAt must be an ISO timestamp")
  ...
  if (FORBIDDEN_KEY_PATTERN.test(serialized)) errors.push("manifest contains forbidden model/config dimension")
  if (FORBIDDEN_MODEL_ID_PATTERN.test(serialized)) errors.push("manifest contains forbidden provider model ID")
...
function validateEntry(entry: SyncedAgentManifestEntry, errors: string[]): void {
  ...
  if (!isIsoDateTime(entry.syncedAt)) errors.push(`${entry.slug}: syncedAt must be an ISO timestamp`)
  if (typeof entry.prompt === "string" && entry.prompt.trim().length === 0) errors.push(`${entry.slug}: prompt must be non-empty when present`)
}
