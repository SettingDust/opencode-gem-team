...
- 该 NOTICE 也会提醒编排器在委派时对照自身已定义的 agent 角色与职责来选择子 agent。
...
`gem-orchestrator` 的权限注入是严格的 delegation-first 约束：`read` / `grep` / `glob` 也必须在权限层被 `deny`，仅放行编排工件路径（`docs/plan/**`、`**/.gem-team.yaml`、`**/AGENTS.md`）。这里必须使用 `deny` 而不是 `ask`，这样在命中非编排源码读取时会返回 `permission_denied` 并继续走委派，而不是弹出交互式确认并中断当前 turn。
