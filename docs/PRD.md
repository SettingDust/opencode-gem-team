...
- 仅对 `gem-orchestrator` 注入默认 `permission`，保留 `permission_denied` 作为明确的委派信号；若用户为 `gem-orchestrator` 自定义了 `permission`，插件不得覆盖。OpenCode 无独立 `write` 权限键，`edit` 规则同时管控 `edit` 与 `write` 工具；`grep` / `glob` 不再通过 permission 条目声明，而由插件 `tool.execute.before` hook 强制限制到 `docs/plan/**`。
...
`gem-orchestrator` 的 delegation-first 约束分两层执行：`read` 继续在 permission 层被 `deny`，仅放行编排工件路径（`docs/plan/*`、`**/.gem-team.yaml`、`**/AGENTS.md`）；`grep` / `glob` 则不再声明 permission 条目，而由插件 `tool.execute.before` hook 强制限制到 `docs/plan/**`。这里必须保持非交互式阻止语义，避免弹出确认并中断当前 turn。
